...
async function paginatedData(page) {
	try {
		const userData = await axios.get(
			`http://3.25.151.168:4000/user/ispremium`,
			{
		...
		const expenses = await axios.get(
			`http://3.25.151.168:4000/expenses/get-expense`,
			{
				headers: {
...
async function addExpense(e) {
	...
	try {
		const response = await axios.post(
			`http://3.25.151.168:4000/expenses/add-expense`,
			expenseObj,
			{
				headers: {
					Authorization: localStorage.getItem("accessToken"),
	...
	if (e.target.classList.contains("del")) {
		try {
			const response = await axios.delete(
				`http://3.25.151.168:4000/expenses/delete-expense/${e.target.parentElement.id}`,
				{
					headers: { Authorization: localStorage.getItem("accessToken") },
				}
			);
...
async function buyPremium(e) {
	...
	const response = await axios.get(
		`http://3.25.151.168:4000/premium/buy-premium`,
		{
			headers: {
	...
	const options = {
		...
		handler: async (response) => {
			...
			await axios.post(
				`http://3.25.151.168:4000/premium/update-txn-status`,
				{
